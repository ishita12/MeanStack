...
module.exports = (router) => {
  ...
  router.post('/register', (req, res) => {
    ...
    if(!req.body.role){
      res.json({success: false, message:'you must select a role'});
    } else {
      if (!req.body.email) {
...
User.findOne({ _id: req.decoded.userId}).select('role').exec((err, user) => {
  if (err) {
    console.log('2');
    res.json({ success: false, message: err }); // Return error
  } else {
    ...
    if (!user) {
      ...
      console.log('user found yayyyy');
      console.log('user role yayyy  '+user.role);
      res.json({ success: true, user: user.role }); // Return success, send user object to frontend for profile
    }
  }
});
